...
NTC_Thermistor::NTC_Thermistor(const int pin,
  ...
  this->initialize(pin, referenceResistance, nominalResistance, nominalTemperatureCelsius, bValue, adcResolution);
}

void NTC_Thermistor::initialize(const int pin,
                                const double referenceResistance,
                                const double nominalResistance,
                                const double nominalTemperatureCelsius,
                                const double bValue,
  ...
  this->adcResolution = adcResolution > 0 ? adcResolution : 0;

  mgos_adc_enable(this->pin);
}
...
inline double NTC_Thermistor::resistanceToKelvins(const double resistance) {
  const double inverseKelvin =
      1.0 / this->nominalTemperature + log(resistance / this->nominalResistance) / this->bValue;
